...
class BlowfishTests: XCTestCase {
    ...
    func testCBCZeroPadding() {
        let key = [UInt8](hex: "0123456789ABCDEFF0E1D2C3B4A59687")
        let iv = [UInt8](hex: "FEDCBA9876543210")
        let input = [UInt8](hex: "37363534333231204E6F77206973207468652074696D6520666F722000")
        XCTAssertEqual(try Blowfish(key: key, blockMode: CBC(iv: iv), padding: .zeroPadding).encrypt(input), [UInt8](hex: "6B77B4D63006DEE605B156E27403979358DEB9E7154616D959F1652BD5FF92CC"))
    }

    func testEncryptDecrypt() {
        let key = [UInt8].init(hex: "0123456789ABCDEFF0E1D2C3B4A59687")
        let iv = [UInt8].init(hex: "FEDCBA9876543210")
        let input = [UInt8].init(hex: "37363534333231204E6F77206973207468652074696D6520666F722000")
