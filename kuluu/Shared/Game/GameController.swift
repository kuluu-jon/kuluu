    ...
    private var activeCamera: SCNNode? {
        didSet {
            ...
            let keepAltitude = SCNTransformConstraint.positionConstraint(inWorldSpace: true, with: {(_ node: SCNNode, _ position: SCNVector3) -> SCNVector3 in
                var position = float3(self.character!.node.position)
...
//        let collisionsScene = SCNScene( named: "Art.scnassets/zones/ssandoria.scn" )
//        collisionsScene!.rootNode.enumerateChildNodes { (_ child: SCNNode, _ _: UnsafeMutablePointer<ObjCBool>) in
//            child.opacity = 0.9
//
////            child.scale = SCNVector3(float3(100))
...
//        }
    ...
    func setupFollowCamera(_ cameraNode: SCNNode) {
        ...
        follow.minimumDistance = 1.3
        follow.maximumDistance = max(follow.minimumDistance, distance)
        cameraNode.camera?.focusDistance = distance * 1.2
        ...
        cameraNode.constraints = [keepAltitude, follow, accelerationConstraint, orientationUpdateConstraint, lookAtConstraint]
...
//        node.simdScale = SIMD3<Float>(x: 1/137.81, y: 1/137.81, z: 1/137.81)
    ...
    func setupCamera() {
        ...
        self.lookAtTarget.name = "camLookAt"
...
//        }
    ...
    init(sceneRenderer scnView: SCNView, ffxiClient: FFXIClient) {
        ...
        scnView.debugOptions = [/*.showPhysicsFields, .showPhysicsShapes, .showSkeletons*/]
        // setup overlay
        overlay = Overlay(size: scnView.bounds.size, controller: self)
        scnView.overlaySKScene = overlay
        
        Task {
            let scene = SCNScene(named: "FFXI.scnassets/ffxi.scn")
            await MainActor.run {
                ...
                light!.castsShadow = false  // turn on cascade shadows
                                            //        light!.shadowMapSize = CGSize(width: CGFloat(1024), height: CGFloat(1024))
                                            //        light!.maximumShadowDistance = 5
    ...
    func addFriends(_ count: Int) {
        ...
        let friendScene = SCNScene(named: "Art.scnassets/character/hume/hume.scn")
        guard let friendModel = friendScene?.rootNode.childNode(withName: "rootNode", recursively: true) else { return }
    ...
    func renderer(_ renderer: SCNSceneRenderer, updateAtTime time: TimeInterval) {
        ...
        if time - lastSyncTime >= 0.5 {
            lastSyncTime = time
            let pos = self.character!.node!.simdPosition
